...
<body>
    <div class="container">
        ...
        <div class="form-group">
            <form method="post">
                ...
                <button class="btn btn-primary" type="button" onclick="generatePDF()">Generate pdf</button>
            </form><br>
        </div>
    ...
    <script>
        function generatePDF() {
            var employeeName = '<?php echo $user['first_name'] . '_' . $user['last_name']; ?>';
            var currentDate = new Date().toLocaleDateString();
            var reportDiv = document.getElementById('report');
            var options = {
                filename: 'Report_' + employeeName + '_' + currentDate + '.pdf',
                margin: 2,
                jsPDF: {format: 'a4', orientation: 'portrait'},
                html2canvas: {scale: 2},
                pagebreak: {mode: ['avoid-all', 'css'], before: '.page-break'}
            };
            options.pagebreak.before = '.page-break';
